...
class CnblogsSpider(CrawlSpider):
    ...
    rules = [
        ...
        #                  callback='parse_item')
        # ]
        Rule(sle(allow=("/ybjourney/default.html\?page=\d{1,}")), #此处要注意?号的转换，复制过来需要对?号进行转义。
                         follow=True,
                         callback='parse_item')
    ]
    ...
    def parse_item(self, response):
        ...
        postTitle = sel.css('div.post div.postTitle')
        #print "=============length======="
        postCon = sel.css('div.post div.c_b_p_desc')
